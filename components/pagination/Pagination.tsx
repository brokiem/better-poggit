...
export default function Pagination({ plugins, currentPage }: { plugins: Plugin[], currentPage: string }) {
  ...
  for (let i = startPage; i <= endPage; i++) {
    ...
    if (i === currentPageNum) {
      pageButtons.push(
        <a key={i} href={"/plugins/page/" + i} className="flex items-center z-10 px-3 py-2 leading-tight text-blue-600 border border-blue-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">
          {i}
        </a>
      );
    } else {
